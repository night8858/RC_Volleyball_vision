...
void grab_img_hikcam(void)
{
    ...
    while (state.load())
    {
        ...
        if (hik_cam.get_one_frame(frame_hik, 0))
        {
            detector.hik_img_flag = E_CAMERA_RESULT::SUCCESS;
        }
        else
        {
            detector.hik_img_flag = E_CAMERA_RESULT::CAMERA_ERROR;
...
void grab_img_usbcam(void)
{
    ...
    while (state.load())
    {
        auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - last_valid_time);
...
int main()
{
    ...
    std::thread grab_hikcam_thread = std::thread(grab_img_hikcam);
    ...
    {
        sleep(1000);
        if (grab_usbcam_thread.joinable() && detect_thread.joinable() && serial_thread.joinable() && grab_hikcam_thread.joinable())
        {
            state.store(false);
            break;
        }
